...
#                 addrDir = dirName.replace('/input_problematic_jpg', '/input_all_dicom')
#                 addrFile = filename.replace('.jpg', '.dcm')
#                 dcm_files.append(os.path.join(addrDir, addrFile))
...
with tqdm(total=len(dcm_files)) as t:
    for n, dcm_file in enumerate(dcm_files):
        ...
        addr = addr.replace('/input_sample_dicom/', '/input_test_png/')
        plt.savefig(addr)
        plt.close()
